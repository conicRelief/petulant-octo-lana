 ...
 */
public class SimpleMovingVariance implements Observable.Operator<Double, Double> {
  private int frame;
  public SimpleMovingVariance(int frame)
  {this.frame = frame;}
  public SimpleMovingVariance()
  {this.frame = 10;}
    ...
    return new Subscriber<Double>() {
      ...
      public void onNext(Double aDouble) {
        ...
        if (counter < frame) {
          ...
          sum = sum(dubList);
          Log.d("SMV MOVING SUM", String.valueOf(sum));
          average = sum / (frame);
          Log.d("SMV MOVING AVERAGE", String.valueOf(average));
          standardDeviationSum = square_difference_sum(dubList,average);
          Log.d("SMV MOVING SUM", String.valueOf(standardDeviationSum));
          variance = standardDeviationSum/frame;
          subscriber.onNext(new Double(variance));        //Non Primitive Overhead
        }
      }
    };
  }
